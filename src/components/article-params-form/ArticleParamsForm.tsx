...
const normalizeFontSizeOption = (value: string | OptionType | null): OptionType => {
  if (!value) {
    return fontSizeOptionsCustom[0];
  }
  if (typeof value === 'string') {
    return fontSizeOptionsCustom.find(opt => opt.value === value) || fontSizeOptionsCustom[0];
  }
  return value;
};
...
const ArticleParamsForm: React.FC<Props> = ({ initialState, isSidebarOpen, onApply, onReset }) => {
  ...
  const handleChange = (key: keyof ArticleStateType, value: OptionType) => {
    setFormState((prev) => {
      ...
      setIsFormChanged(JSON.stringify(newState) !== JSON.stringify(initialState));
      return newState;
    });
  };
  ...
  <div className={styles.formContent}>
    ...
    <div className={styles.bottomContainer}>
      <Button title="Сбросить" htmlType="button" type={isFormChanged ? 'apply' : 'clear'} onClick={handleReset} />
      <Button title="Применить" htmlType="button" type={isFormChanged ? 'apply' : 'clear'} onClick={handleApply} />
    </div>
  </div>
</form>
    </aside>
